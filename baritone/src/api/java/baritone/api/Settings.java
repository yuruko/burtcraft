...
public final class Settings {
    ...
    public final Setting<List<Block>> blocksToAvoid = new Setting<>(new ArrayList<>(
            // leave empty by default - altoclef drives avoidance through AltoClefSettings
    ));
     ...
     * Render the path
     * <p>
     * burnt fork: render defaults are OFF - the path/goal/selection overlays are
     * debug chrome that must never cover her stream. a regenerated settings file
     * keeps them hidden.
     */
    public final Setting<Boolean> renderPath = new Setting<>(false);
     ...
     * Render the goal
     */
    public final Setting<Boolean> renderGoal = new Setting<>(false);
     ...
     */
    public final Setting<Boolean> renderGoalAnimated = new Setting<>(false);

    /**
     * Render selection boxes
     */
    public final Setting<Boolean> renderSelectionBoxes = new Setting<>(false);
     ...
     */
    public final Setting<Integer> smoothLookTicks = new Setting<>(2);

    /**
     * Cosmetic camera smoothing for the burnt stream. When on, the RENDERED camera eases toward the
     * look target during travel/idle (SERVER-mode look) instead of freezing or snapping, so on stream
     * the head-turn reads as a smooth pan. Purely visual: the aim sent to the server is still exact
     * ({@link baritone.behavior.LookBehavior#onPlayerRotationMove}), and block interactions use CLIENT
     * mode + an instant snap (which mining alignment depends on), so this never affects pathing or
     * mining accuracy. Set false to restore the old frozen/snapped camera.
     */
    public final Setting<Boolean> smoothCameraFollow = new Setting<>(true);

    /**
     * Max degrees the cosmetic {@link #smoothCameraFollow} camera may turn per tick. Higher = snappier
     * (closer to the old instant camera), lower = slower, more cinematic pan. A 180 degree turn takes
     * ceil(180 / this) ticks. Only affects the rendered camera, never the sent aim.
     */
    public final Setting<Double> cameraFollowMaxDegreesPerTick = new Setting<>(8.0d);

    /**
     * Keep the RENDERED pitch near the horizon while travelling, so the bot appears to look where it is
     * walking instead of at its own feet. Baritone aims at the next foothold, which on flat ground is a
     * block at ground level - correct for pathing, but on stream it reads as a character permanently
     * staring at the dirt. Cosmetic only, same guarantees as {@link #smoothCameraFollow}: applied in the
     * POST pass after movement is computed, the aim sent to the server is unchanged, and block
     * interactions (CLIENT-mode snap, which mining alignment depends on) never go through it.
     */
    public final Setting<Boolean> travelLookAhead = new Setting<>(true);

    /**
     * Furthest below the horizon, in degrees, the cosmetic travel camera will settle while walking.
     * Anything steeper is eased back toward this. Larger = more head-down. Only used when
     * {@link #travelLookAhead} is on, and never during block interaction or elytra flight.
     */
    public final Setting<Double> travelMaxPitchDown = new Setting<>(12.0d);
