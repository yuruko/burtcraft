...
public class CreateDistTask extends BaritoneGradleTask {
    ...
    protected void exec() throws Exception {
        ...
        // copy build jars to dist/ - the api/standalone pair only exists when proguard
        // actually ran (see ProguardTask: a jdk image with no /jmods skips it), so copy
        // whatever is on disk instead of blowing up on the missing ones
        copyIfPresent(this.artifactApiPath, api);
        copyIfPresent(this.artifactStandalonePath, standalone);
        copyIfPresent(this.artifactUnoptimizedPath, unoptimized);
    ...
    private void copyIfPresent(Path from, Path to) throws Exception {
        if (Files.exists(from)) {
            Files.copy(from, to, REPLACE_EXISTING);
        } else {
            getLogger().warn("dist: skipping {} (not built)", getFileName(from));
        }
    }
