...
public class GoalDodgeProjectiles implements Goal {
    ...
    private static boolean isInvalidProjectile(CachedProjectile projectile) {
        ...
        //if (projectile.getVelocity().lengthSqr() < 0.1) return false;
        return false;
    }
        ...
        synchronized (BaritoneHelper.MINECRAFT_LOCK) {
            if (!projectiles.isEmpty()) {
                for (CachedProjectile projectile : projectiles) {
                    ...
                    //double sqFromMob = creepuh.distanceToSqr(x, y, z);
        ...
        synchronized (BaritoneHelper.MINECRAFT_LOCK) {
            if (!projectiles.isEmpty()) {
                for (CachedProjectile projectile : projectiles) {
                    ...
                    //double arrowCost = hit.distanceToSqr(p); //Math.pow(p.x - hit.x, 2) + Math.pow(p.z - hit.z, 2);
