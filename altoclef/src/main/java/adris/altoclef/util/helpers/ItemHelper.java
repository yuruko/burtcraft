...
public class ItemHelper {
    ...
    public static boolean areShearsEffective(Block b) {
        return
                //b.builtInRegistryHolder().streamTags().anyMatch(t -> t ==
    ...
    public static boolean canStackTogether(ItemStack from, ItemStack to) {
        if (to.isEmpty() && from.getCount() <= from.getOrDefault(DataComponents.MAX_STACK_SIZE, 1))
            return true;
        return to.getItem().equals(from.getItem()) && (from.getCount() + to.getCount() < to.getOrDefault(DataComponents.MAX_STACK_SIZE, 1));
    }

    private static Map<Item, Integer> getFuelTimeMap() {
        // 26.1: AbstractFurnaceBlockEntity#getFuel is gone - burn times moved onto the
        // level's FuelValues, so rebuild the old item->ticks map from that. no world means
        // no fuel data yet, so don't cache an empty map or we'd be stuck with it.
        if (_fuelTimeMap == null) {
            Level level = Minecraft.getInstance().level;
            if (level == null) return Collections.emptyMap();
            FuelValues fuel = level.fuelValues();
            Map<Item, Integer> map = new HashMap<>();
            for (Item item : fuel.fuelItems()) {
                map.put(item, fuel.burnDuration(new ItemStack(item)));
            }
            _fuelTimeMap = map;
        }
        return _fuelTimeMap;
