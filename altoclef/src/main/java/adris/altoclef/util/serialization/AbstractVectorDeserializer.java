...
public abstract class AbstractVectorDeserializer<T, UnitType> extends StdDeserializer<T> {
    ...
    public T deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        ...
        if (p.getCurrentToken() == JsonToken.VALUE_STRING) {
            // the VALUE is getText(); getCurrentName() is the FIELD name. reading
            // the name here made every "x,y,z" setting throw
            // (Invalid BlockPos string: "homeBasePosition") which aborts the whole
            // Settings deserialization - so altoclef silently ran on hardcoded
            // defaults and every value in altoclef_settings.json was ignored.
            String bposString = p.getText();
            ...
            while (p.getCurrentToken() != JsonToken.END_OBJECT) {
                if (p.getCurrentToken() == JsonToken.FIELD_NAME) {
                    ...
                    try {
                        // same fix: key is the field name we already captured,
                        // value is the token's text - not the field name twice.
                        parts.put(fName, parseUnit(p.getText()));
                    } catch (Exception e) {
                        throw new JsonParseException(p, "Failed to parse unit " + fName + " = " + p.getText());
                    }
                    p.nextToken();
                } else {
