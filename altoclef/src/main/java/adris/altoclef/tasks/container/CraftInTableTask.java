...
class DoCraftInTableTask extends DoStuffInContainerTask {
    ...
    protected Task containerSubTask(AltoClef mod) {
        ...
        for (RecipeTarget target : _targets) {
            ...
            Optional<RecipeDisplayEntry> recipeToSend = JankCraftingRecipeMapping.getMinecraftMappedRecipe(target.getRecipe(), target.getOutputItem());

            // Get the client player entity
            LocalPlayer player = Minecraft.getInstance().player;

            // If crafting book is enabled and the recipe to send exists, return a CraftGenericWithRecipeBooksTask.
            // 26.1: the mapping is built FROM the player's own recipe book, so a hit already means
            // they know it - the old RecipeBook#contains check is gone and no longer needed.
