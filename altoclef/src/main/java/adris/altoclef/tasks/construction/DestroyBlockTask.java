...
public class DestroyBlockTask extends Task implements ITaskRequiresGrounded {
    ...
    private void handleNonEmptyCursorStack(AltoClef mod, ItemStack cursorStack) {
        ...
        mod.getItemStorage().getSlotThatCanFitInPlayerInventory(cursorStack, false)
                .ifPresent(slot -> mod.getSlotHandler().clickSlot(slot, 0, ContainerInput.PICKUP));
    ...
    protected Task onTick(AltoClef mod) {
        ...
        if (mod.getWorld().getBlockState(_pos).getBlock() == Blocks.WHITE_WOOL) {
            ...
            for (Entity entity : entities) {
                // Check if the entity is a Pillager and is within a distance of 144 blocks from the position
        ...
        if (reach.isPresent() && (mod.getPlayer().isInWater() || mod.getPlayer().onGround()) && !mod.getFoodChain().needsToEat() && !WorldHelper.isInNetherPortal(mod) && mod.getClientBaritone().getPathingBehavior().isSafeToCancel()) {
