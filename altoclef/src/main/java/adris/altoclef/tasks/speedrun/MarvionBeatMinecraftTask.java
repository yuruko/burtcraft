...
public class MarvionBeatMinecraftTask extends Task {
    ...
    private static final Predicate<ItemStack> _noCurseOfBinding = stack -> {
        return stack.getEnchantments().keySet().stream().noneMatch(e -> e.is(Enchantments.BINDING_CURSE));
    };
    ...
    private static List<BlockPos> getFrameBlocks(BlockPos endPortalCenter) {
        ...
        if (endPortalCenter != null) {
            ...
            for (int[] offset : frameOffsets) {
                ...
                BlockPos frameBlock = endPortalCenter.offset(offset[0], offset[1], offset[2]);
    ...
    public boolean isFinished(AltoClef mod) {
        // Check if the current screen is the WinScreen
        if (getInstance().screen instanceof WinScreen) {
            return true;
        }
    ...
    private Optional<BlockPos> locateClosestUnopenedRuinedPortalChest(AltoClef mod) {
        ...
        try {
            return mod.getBlockTracker().getNearestTracking(blockPos -> {
                ...
                boolean isWithinDistance = mod.getPlayer().blockPosition().closerThan(blockPos, 150);
    ...
    protected Task onTick(AltoClef mod) {
        if (mod.getPlayer().getMainHandItem().getItem() instanceof EnderEyeItem &&
                !openingEndPortal) {
        ...
        if (!craftingTables.isEmpty()) {
            for (BlockPos craftingTable : craftingTables) {
                ...
                if (!mod.getBlockTracker().unreachable(craftingTable)) {
                    BlockState craftingTablePosUp = mod.getWorld().getBlockState(craftingTable.above(2));
                    if (mod.getEntityTracker().entityFound(Witch.class)) {
                        Optional<Entity> witch = mod.getEntityTracker().getClosestEntity(Witch.class);
                        if (witch.isPresent()) {
                            if (craftingTable.closerToCenterThan(witch.get().position(), 15)) {
                ...
                for (Entity entity : entities) {
                    if (entity instanceof Pillager) {
                        if (!mod.getBlockTracker().unreachable(log)) {
                            if (log.closerToCenterThan(entity.position(), 40)) {
                ...
                for (Entity entity : entities) {
                    if (entity instanceof Enemy) {
                        if (!mod.getBlockTracker().unreachable(deepslateCoalOre.get())) {
                            if (mod.getPlayer().distanceToSqr(entity.position()) < 150 &&
                                    deepslateCoalOre.get().closerToCenterThan(entity.position(), 30)) {
                                if (!ironGearSatisfied && !eyeGearSatisfied) {
                ...
                for (Entity entity : entities) {
                    if (entity instanceof Enemy) {
                        if (!mod.getBlockTracker().unreachable(coalOrePos.get())) {
                            if (mod.getPlayer().distanceToSqr(entity.position()) < 150 &&
                                    coalOrePos.get().closerToCenterThan(entity.position(), 30)) {
                                if (!ironGearSatisfied && !eyeGearSatisfied) {
                                    Debug.logMessage("Blacklisting dangerous coal ore.");
                ...
                for (Entity entity : entities) {
                    if (entity instanceof Enemy) {
                        if (!mod.getBlockTracker().unreachable(deepslateIronOrePos.get())) {
                            if (mod.getPlayer().distanceToSqr(entity.position()) < 150 &&
                                    deepslateIronOrePos.get().closerToCenterThan(entity.position(), 30)) {
                                if (!ironGearSatisfied && !eyeGearSatisfied) {
                ...
                for (Entity entity : entities) {
                    if (entity instanceof Enemy) {
                        if (!mod.getBlockTracker().unreachable(ironOrePos.get())) {
                            if (mod.getPlayer().distanceToSqr(entity.position()) < 150 &&
                                    ironOrePos.get().closerToCenterThan(entity.position(), 30)) {
                                if (!ironGearSatisfied && !eyeGearSatisfied) {
                                    Debug.logMessage("Blacklisting dangerous iron ore.");
        ...
        if ((_logsTask != null || _foodTask != null || _getOneBedTask.isActive() || _stoneGearTask != null ||
            ...
            if (!mod.getClientBaritone().getExploreProcess().isActive()) {
                ...
                if (_timer3.elapsed()) {
                    if (_config.renderDistanceManipulation) {
                        getInstance().options.renderDistance().set(20);
            ...
            if (!shears.isEmpty() && !needsBeds(mod)) {
                for (Slot shear : shears) {
                    if (Slot.isCursor(shear)) {
                        ...
                        mod.getSlotHandler().clickSlot(shear, 0, ContainerInput.PICKUP);
                    }
                }
            }
            if (!furnaceSlots.isEmpty() && mod.getItemStorage().hasItem(Items.SMOKER) &&
                ...
                for (Slot furnace : furnaceSlots) {
                    if (Slot.isCursor(furnace)) {
                        ...
                        mod.getSlotHandler().clickSlot(furnace, 0, ContainerInput.PICKUP);
                    }
                }
            }
            if (!sands.isEmpty()) {
                for (Slot sand : sands) {
                    if (Slot.isCursor(sand)) {
                        ...
                        mod.getSlotHandler().clickSlot(sand, 0, ContainerInput.PICKUP);
                    }
                }
            }
                ...
                if (!gravels.isEmpty()) {
                    for (Slot gravel : gravels) {
                        if (Slot.isCursor(gravel)) {
                            ...
                            mod.getSlotHandler().clickSlot(gravel, 0, ContainerInput.PICKUP);
                        }
                    }
                }
            }
            if (!torches.isEmpty()) {
                for (Slot torch : torches) {
                    if (Slot.isCursor(torch)) {
                        ...
                        mod.getSlotHandler().clickSlot(torch, 0, ContainerInput.PICKUP);
                    }
                }
            }
                ...
                if (!excessWaterBuckets.isEmpty()) {
                    for (Slot excessWaterBucket : excessWaterBuckets) {
                        if (Slot.isCursor(excessWaterBucket)) {
                            ...
                            mod.getSlotHandler().clickSlot(excessWaterBucket, 0, ContainerInput.PICKUP);
                        }
                    }
                }
            }
                ...
                if (!excessLighters.isEmpty()) {
                    for (Slot excessLighter : excessLighters) {
                        if (Slot.isCursor(excessLighter)) {
                            ...
                            mod.getSlotHandler().clickSlot(excessLighter, 0, ContainerInput.PICKUP);
                        }
                    }
                }
            }
                ...
                if (!beds.isEmpty()) {
                    for (Slot bed : beds) {
                        if (Slot.isCursor(bed)) {
                            ...
                            mod.getSlotHandler().clickSlot(bed, 0, ContainerInput.PICKUP);
                        }
                    }
                }
            }
                        ...
                        if (!throwGears.isEmpty()) {
                            for (Slot throwGear : throwGears) {
                                if (Slot.isCursor(throwGear)) {
                                    ...
                                    mod.getSlotHandler().clickSlot(throwGear, 0, ContainerInput.PICKUP);
                                }
                            }
                        }
                        if (!ironArmors.isEmpty()) {
                            for (Slot ironArmor : ironArmors) {
                                if (Slot.isCursor(ironArmor)) {
                                    ...
                                    mod.getSlotHandler().clickSlot(ironArmor, 0, ContainerInput.PICKUP);
                                }
                            }
                        }
                    }
    ...
    private Task getEnderPearlTask(AltoClef mod, int count) {
        ...
        return new SearchWithinBiomeTask(Biomes.WARPED_FOREST);
    }
    ...
    private boolean canBeLootablePortalChest(AltoClef mod, BlockPos blockPos) {
        ...
        if (mod.getWorld().getBlockState(blockPos.above()).getBlock() == Blocks.WATER ||
                blockPos.getY() < 50) {
            return false;
        }

        // Define the minimum and maximum positions for scanning
        BlockPos minPos = blockPos.offset(-4, -2, -4);
        BlockPos maxPos = blockPos.offset(4, 2, 4);
        ...
        switch (WorldHelper.getCurrentDimension()) {
            case OVERWORLD -> {
                ...
                if (!mod.getItemStorage().hasItem(Items.PORKCHOP) &&
                    ...
                    if (mod.getEntityTracker().entityFound(Pig.class) && (StorageHelper.itemTargetsMet(mod,
