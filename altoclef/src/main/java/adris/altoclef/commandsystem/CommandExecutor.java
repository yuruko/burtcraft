...
public class CommandExecutor {
    ...
    // the external control companion reaches altoclef through the static
    // getCommandExecutor(); expose the mod so it can read the live task runner
    // (the "what am i actually doing right now" readout) without a second static.
    ...
    // like executeWithPrefix but carries onFinish/onError back to the caller (used by the
    // external control server so burnt's commands run under whatever prefix is actually
    // configured - default "@", but often "." - instead of a hardcoded one that silently
    // no-ops in execute() when it doesn't match.
    public void executeWithPrefix(String line, Runnable onFinish, Consumer<CommandException> getException) {
        if (!line.startsWith(getCommandPrefix())) {
            line = getCommandPrefix() + line;
        }
        execute(line, onFinish, getException);
    }
