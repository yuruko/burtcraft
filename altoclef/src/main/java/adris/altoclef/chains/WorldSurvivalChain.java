...
public class WorldSurvivalChain extends SingleTaskChain {
    ...
    public float getPriority(AltoClef mod) {
        ...
        if (mod.getModSettings().shouldExtinguishSelfWithWater()) {
            if (!(_mainTask instanceof EscapeFromLavaTask && isCurrentlyRunning(mod)) && mod.getPlayer().isOnFire() && !mod.getPlayer().hasEffect(MobEffects.FIRE_RESISTANCE) && !mod.getWorld().environmentAttributes().getDimensionValue(EnvironmentAttributes.WATER_EVAPORATES)) {
