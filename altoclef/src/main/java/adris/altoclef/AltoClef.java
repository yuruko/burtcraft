...
public class AltoClef implements ModInitializer {
    ...
    // guards onInitializeLoad so it runs exactly once, whether triggered by the title
    // screen or by the quickplay client-tick bootstrap (see onInitialize)
    ...
    public void onInitialize() {
        ...
        // fallback for when the title screen is skipped - e.g. launching straight into a
        // world with --quickPlaySingleplayer. in that case TitleScreenEntryEvent never
        // fires and altoclef would never initialize, so bootstrap off the client tick:
        // the first tick that has a player present runs the load exactly once.
        EventBus.subscribe(ClientTickEvent.class, evt -> {
            if (!_loaded && Minecraft.getInstance().player != null) onInitializeLoad();
        });
    }
    ...
    public void onInitializeLoad() {
        ...
        if (_loaded) return; // only ever initialize once (title screen OR quickplay tick)
        _loaded = true;
    ...
    private void initializeBaritoneSettings() {
        ...
        // freeLook keeps baritone from yanking the camera down to stare at each path node -
        // she reads far more like a person walking around than a bot studying its feet.
        getClientBaritoneSettings().freeLook.value = true;
        // Keep exact break/place aim on the server-side rotation too. LookBehavior
        // remains the sole owner of the rendered camera and can ease toward that
        // target instead of snapping down/up once per block.
        getClientBaritoneSettings().blockFreeLook.value = true;
        getClientBaritoneSettings().smoothCameraFollow.value = true;
        getClientBaritoneSettings().cameraFollowMaxDegreesPerTick.value = 8.0;
